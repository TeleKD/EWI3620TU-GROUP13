...
public class PlayMenu extends MenuObject{
	...
	public PlayMenu(int minX,int maxX,int minY,int maxY){
		...
		buttons[0] = new Button("New",minX,maxX,minY+(maxY-minY)*2/3,maxY,0f,1f,.5f);
		buttons[1] = new Button("Load",minX,maxX,minY+(maxY-minY)/3,minY+(maxY-minY)*2/3,0f,1f,.5f);
		buttons[2] = new Button("Back",minX,maxX,minY,minY+(maxY-minY)/3,0f,1f,.5f);
	}
