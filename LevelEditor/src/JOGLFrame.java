...
public class JOGLFrame extends Frame implements GLEventListener, MouseListener {
	...
	private void drawButtons(GL gl) {
		...
		gl.glVertex2f (3 * buttonSize - 8.0f, screenHeight - buttonSize + 8.0f); 
		gl.glVertex2f (2 * buttonSize + 8.0f, screenHeight - buttonSize + 8.0f); 
		gl.glEnd();
	}
	...
	private void drawFigure(GL gl) {
		...
		case DM_KOCH:
			if (points.size() >= 3) {
				...
				// three points, draw a kochline between each pair of points
				p1 = points.get(0);
				p2 = points.get(1);
				p3 = points.get(2);
				drawKoch(gl, p1.x, p1.y, p2.x, p2.y, 5);
				drawKoch(gl, p2.x, p2.y, p3.x, p3.y, 5);
				drawKoch(gl, p3.x, p3.y, p1.x, p1.y, 5);
			}
			break;
		}
	}
	
	//Used to draw kochlines of the nth order
	private void drawKoch(GL gl, float x1, float y1, float x2, float y2, int n){
		float lijn = (float) Math.sqrt((x2-x1)*(x2-x1)+(y2-y1)*(y2-y1));
		float hoek = 60*(float)Math.PI/180;
		float hoeklijn;
		
		if (x2>x1)
		hoeklijn = (float)Math.atan((y2-y1)/(x2-x1));
		else
		hoeklijn = (float)Math.atan((y2-y1)/(x2-x1))+(float)Math.PI;	
		...
		//clockwise: && x2 > x1
			...
			float x4 = x1+((x2-x1)/3*2);
			float y4 = y1+((y2-y1)/3*2);
			
			float x5 = x3+((float)Math.cos(hoeklijn-hoek)*(lijn/3));
			float y5 = y3+((float)Math.sin(hoeklijn-hoek)*(lijn/3));
			...
			*/
			...
			drawKoch(gl, x3, y3, x5, y5, n-1);
			drawKoch(gl, x5, y5, x4, y4, n-1);
			drawKoch(gl, x1, y1, x3, y3, n-1);
			drawKoch(gl, x4, y4, x2, y2, n-1);
		}
